...
def download_youtube_audio(youtube_url):
    with youtube_dl.YoutubeDL() as ydl:
        ...
        ydl_opts = {
            ...
            'outtmpl': sanitized_title + '.%(ext)s',  # Updated filename template
        }

    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        ydl.download([youtube_url])
        return sanitized_title + '.mp3'
...
# Function to add a sound file to Soundpad without specifying a category
def add_sound_to_soundpad(file_path):
    handle = connect_to_soundpad()
    if handle:
        # Escape backslashes and quote the file path
        escaped_file_path = file_path.replace("\\", "\\\\")
        quoted_file_path = f'"{escaped_file_path}"'
